  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      function dropAll() {
        ...
        // 0:02.0 cookies: the pit owns the cookie prompt on "/". Tapping it opens
        // the CookieDrop notice (pc:open-cookies, text only there) and squeezes out
        // the pit's own Accept/Reject objects. On every other page the DOM
        // FallingCookie plays this role; keyboard, screen-reader and reduced-motion
        // users get the notice directly.
      ...
      const tapButton = (pt: { x: number; y: number }) => {
        ...
        if (!hit) return false;
        if (hit.plugin?.kind === "cookiereject-inert") return true; // swallow tap -- cannot be collected
...
if (hit.plugin?.kind === "cookieaccept") { cookieBannerOpenRef.current = false;
          ...
          fxKit.pushBurst({ x: ax, y: ay, s: asz, born: bt, life: 480, colour: "#ff2d78", rot: 0 });
          fxKit.pushBurst({ x: ax, y: ay, s: asz * 0.66, born: bt, life: 480, colour: "#ffd23e", rot: 18 });
          numAt(ax, ay, 2000);
        ...
        if (hit.plugin?.kind === "cookiereject") { cookieBannerOpenRef.current = false;
          ...
          fxKit.pushBurst({ x: rx, y: ry, s: rsz, born: bt, life: 460, colour: "#0c5b92", rot: 0 });
          fxKit.pushBurst({ x: rx, y: ry, s: rsz * 0.66, born: bt, life: 460, colour: "#9a9a9a", rot: 18 });
          clearCookieObjects();
          return true;
        }
      ...
      const openLineageAt = (up: { x: number; y: number }) => {
        ...
        if (!hit.plugin.prop && hit.plugin.kind !== "pct" && hit.plugin.kind !== "reserve" && hit.plugin.kind !== "preorder" && hit.plugin.kind !== "menu" && hit.plugin.kind !== "cookieaccept" && hit.plugin.kind !== "cookiereject" && hit.plugin.kind !== "cookiereject-inert" && hit.plugin.kind !== "rod" && hit.plugin.kind !== "pill" && !hit.plugin.collected) {
      ...
      // The blast effects now live in pitEffects.ts and are shared with the mini
      // pit, so the two can never drift. Destructured so every existing call
      // site below reads exactly as it did.
      const fxKit = createPitEffects();
      const { burstAt, explodeAt, pushBoom, emitFuseSparks, FUSE_LIGHT_AT } = fxKit;
      ...
      const onAfter = () => {
        ...
        if (lineageOpenRef.current) { for (const b of bodies) drawBall(ctx, b, 1, false); drawParticles(ctx, now); fxKit.draw(ctx, now); drawNumbers(ctx, now); return; }
    ...
    >
      ...
      {britainMsg !== null && (
        <BritainMessage
          onDismiss={() => {
            setBritainMsg(null);
            // poof the flag via a custom event the physics loop listens for
