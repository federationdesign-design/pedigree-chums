...
test("undiscovered game names are absent from the DOM and accessibility tree", async ({ page }) => {
  await page.goto("/about");
  await expect(page.getByText(/0\/2 games found/)).toBeVisible();
...
test("reduced motion: the completion celebration is a static highlight, no animation", async ({ page }) => {
