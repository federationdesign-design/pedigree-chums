...
import { STORAGE_KEY, TARGET } from "../../lib/hiddenGames/registry.ts";
...
test("HG-ENG-01 a fresh visitor restores to 0 of the fixed target", () => {
  const store = makeStore();
  const { engine } = makeEngine(store);
  const s = engine.getState();
  assert.equal(s.count, 0);
  assert.equal(s.total, TARGET); // the counter total is the fixed target (10), not games.length
  assert.equal(s.label, `0/${TARGET} games found`);
});
...
test("HG-RESTORE-02 an existing valid record is reflected before any report or render", () => {
  ...
  assert.equal(engine.getState().label, `1/${TARGET} games found`); // count from the seeded record, total is the fixed target
