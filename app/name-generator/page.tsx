                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
                            {qItem.options.map((opt: {label:string;bonus:string[]}, optIdx: number) => (
                              <button key={opt.label}
                                ...
                                style={{ padding:"10px 16px", borderRadius:999, border:`1.5px solid ${chosen===opt.label?"var(--yellow)":"rgba(255,255,255,0.18)"}`, background:chosen===opt.label?"var(--yellow)":"rgba(255,255,255,0.05)", color:chosen===opt.label?"var(--navy)":"rgba(255,255,255,0.85)", fontFamily:"var(--font-body)", fontSize:"0.78rem", fontWeight:700, cursor:"pointer", textAlign:"center", lineHeight:1.2, transition:"background 0.15s, border-color 0.15s, color 0.15s", animation:"pcAnswerIn 0.35s ease both", animationDelay:`${(pos*0.18)+(optIdx*0.09)}s` }}>
                                {opt.label}
                              </button>
                            ))}
                          </div>
