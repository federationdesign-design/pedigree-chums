...
function generateScored(breed: string, surname: string, gender: "boy"|"girl", seed: number, town = "", colour: DogColour = "", excludeDominant = false, freeRange = false, allowBonus: Set<string> = new Set()) {
  ...
  // allowBonus names always pass through even when excludeDominant is true
  // This means answering "Yes I like space" actually un-gates Luna/Astro/Nova
  const nameBank = excludeDominant
    ? (rawNameBank.filter((n: NameEntry) => !DOMINANT_NAMES.has(n.name) || allowBonus.has(n.name)).length >= 3
        ? rawNameBank.filter((n: NameEntry) => !DOMINANT_NAMES.has(n.name) || allowBonus.has(n.name))
        : rawNameBank)
    : rawNameBank;
...
function runPass(
  ...
  const allowBonus = new Set<string>(bonusPool1);
  ...
  const raw = Array.from({length:20}, (_, i) => {
    const r = generateScored(breed, surname, gender, baseSeed + i * 17, town, colour, excludeDominant, freeRange, allowBonus);
    if (!r) return null;
    const parts = r.full.split(" ");
    const fn = parts[1] ?? "";
  ...
  if ((raw[0]?.score ?? 0) < THRESHOLD) {
    ...
    const prefixed = Array.from({length:20}, (_, i) => {
      ...
      const informalTitles = ["Lil'","Baby","Little","Big","Ol'","Daft","Cheeky","Silly","Scruffy","Fluffy","Grumpy","Noisy","Squishy","Itsy","Teeny","Ol'"];
