...
function scoreName(title: TitleEntry, first: NameEntry, dogWord: WordEntry, surname: string, colour: DogColour = ""): number {
  ...
  // ── Booboo -- in 4 pools, massively over-represented ─────────────────────
  if (first.name === "Booboo") score -= 8;

  // ── First name matching surname (William Williams, Norman Norman) ─────────
  const surnameBase2 = surname.replace(/-.*/, "").replace(/s$/i, "").toLowerCase();
  if (first.name.toLowerCase() === surnameBase2 && surnameBase2.length > 2) score -= 10;
