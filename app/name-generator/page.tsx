...
function scoreName(title: TitleEntry, first: NameEntry, dogWord: WordEntry, surname: string, colour: DogColour = ""): number {
  ...
  // ── SPECIAL NAME GUARDRAILS ──────────────────────────────────────────────────
  // These names only work in specific combinations
  const specialNameRules: Record<string, (t: string, w: string) => number> = {
    "Woesworth":   (t, w) => ["Waggle","Bounce","Wiggle","Frolic","Wag","Romp","Prance","Wriggle","Gambol"].includes(w) ? 3 : -4,
    "Nonchalance": (t, w) => ["Bolt","Crash","Blunder","Charge","Yap","Dig","Bark"].includes(w) ? -5 : 2,
    "Frantic":     (t, w) => ["Lil'","Little","Baby","Cheeky","Daft","Silly","Wee"].includes(t) ? 4 : -6,
    "Lamentia":    (t, w) => ["Baron","Baroness","Commissioner","Field Marshal","Archdruid","Emperor","Empress","Magnificent","Legendary"].includes(t) ? -5 : 2,
  };
  const specialRule = specialNameRules[first.name];
  if (specialRule) score += specialRule(title.title, dogWord.word);
...
const NAMES: Record<string, { boy: NameEntry[]; girl: NameEntry[] }> = {
  ...
  sniffer: {
    ...
    girl: [{name:"Woesworth",reg:"absurd",syllables:2},{name:"Lamentia",reg:"absurd",syllables:3},{name:"Booboo",reg:"baby",syllables:2},{name:"Mimi",reg:"baby",syllables:2},{name:"Fifi",reg:"baby",syllables:2},{name:"Snugglebum",reg:"baby",syllables:3},{name:"Cuddlekins",reg:"baby",syllables:3}],
  },
  greatdane: {
...
const MCFACE_POOL: Record<string, [string, string][]> = {
  sniffer:    [["Sniffy","Sniff"],["Droopy","Droop"],["Slobbery","Slobber"],["Lollopy","Lollop"],["Nosey","Nose"],["Houndy","Hound"],["Houndy","Hound"],["Questy","Quest"]],
