...
// Vercel Global Config (formerly Edge Config) so the owner can flip it OFF in the Vercel dashboard and have it
// take effect in SECONDS, with NO redeploy. This is the "stop it instantly" control (deliberately runtime,
// not a build-time env var).
//
// DEFAULT IS OFF (see readSyncConfig). The Apps Script endpoint URL lives in the same Global Config item and
// is deliberately NOT returned here: the client only needs on/off; the URL stays server-side for /api/pc-sync.
//
