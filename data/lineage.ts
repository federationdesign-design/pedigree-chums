...
const LINEAGE: Record<string, LineageNode> = {
  ...
  "Norfolk Terrier": {
    ...
    children: [
      { name: "Norwich Terrier", note: "The same East Anglian red terriers, before the ear-type split.", img: "/history/breeds/norwich-terrier-photo.jpg", value: 50 },
  ...
  "Longdog": {
    ...
    children: [
      ...
      { name: "Scottish Deerhound", note: "Bigger sighthound blood for stamina over rough ground. Now endangered.", img: "/history/breeds/Medieval-Scottish-Deerhound.jpg", value: 25 }
    ]
  },
  ...
  /* THE KING'S WHITE HOUNDS, added 24 September 2026 (owner). The Chiens blancs
     ...
     its own origin is the royal packs' tradition, not a parent to author. */
  "King's White Hounds": {
    name: "King's White Hounds",
    note: "The white scent hounds of the French kings' own packs, the hound the English Staghound is said to have looked most like, from the centuries the two courts gave each other hounds. A likely link rather than a recorded cross. Now extinct.",
    img: "/history/breeds/kings-white-hounds.jpg",
    children: []
  },
  ...
  /* ITS OWN RECORD, 15 September 2026 (owner: it should appear in the ancient
     ...
     THE PARENTS ARE THE OWNER'S, given 15 September once it became a level:
     Celtic Coursing Hound, Celtic Hound and Old hunting dogs of the Celts.

     WHAT THEY REPLACED. The first version lifted the ancestry verbatim from the
     inline copy inside "Livestock Dog", which paired Old hunting dogs of the
     Celts with a SELF-CHILD, the same dog inside itself. That is a normal shape
     for an ancestor circle, and the owner's ruling of the same day is that it is
     deliberate: a circle stands for a dog TYPE, so a type can sit behind itself.
     It stops working the moment the dog becomes a LEVEL, because the player is
     then asked to find the dog they are already looking at, using the same
     artwork, and the diagram has one real circle instead of two.

     The inline copy under Livestock Dog is untouched and keeps its self-child.
     It already has children, so it does not graft, and it is the source the
     first version was taken from.

     THE THIRDS ARE MINE, NOT THE OWNER'S. No split was given, so the three are
     even. One number each if any of them should weigh more. */
  ...
  "Staghound": {
    ...
    children: [
      /* THE KING'S WHITE HOUNDS JOIN, 24 September 2026 (owner). The claim is
         only that the Staghound most closely resembled the French kings' white
         hounds, with hound gifts between the courts as the route: SPECULATIVE,
         so a QUARTER under the split rule. The other three are scaled into the
         remaining 75 and rounded, 34/26/15. */
      { name: "Southern Hound", note: "The heavy deer-hunting scent hound at its base.", img: "/history/breeds/Southern-Hound.jpg", value: 34 },
      { name: "English Foxhound", note: "Pack-hound blood for pace and a long day's work.", img: "/history/breeds/english-foxhound.jpg", value: 26 },
      { name: "Buckhound", note: "The royal deer hound in its background.", img: "/history/breeds/Buckhound-illustration.jpg", value: 15 },
      { name: "King's White Hounds", note: "The white scent hounds of the French kings' own packs, the hound the English Staghound is said to have looked most like, from the centuries the two courts gave each other hounds. A likely link rather than a recorded cross. Now extinct.", img: "/history/breeds/kings-white-hounds.jpg", value: 25 }
    ]
  },
...
const LINEAGE_ALIASES: Record<string, string> = {
  ...
  /* THE DEAD-END MERGES, 15 September 2026 (owner), following the same two-step
     the Talbot entries above describe: the child nodes are renamed in the data
     so the picture and the rarity tier read one name, and the old spellings are
     kept here as no-ops so an older name returning still grafts.

     WHY. An audit of all 160 lineages found 88 names that appear as a leaf with
     ...
     NOTHING WAS INVENTED. Every target on the right already had its own record.
     Two of these are not aliases but duplicates, one dog under two spellings,
     merged onto the owner's chosen name.

     EXPECT PERCENTAGES TO MOVE. A dead end has no children, so it was a
